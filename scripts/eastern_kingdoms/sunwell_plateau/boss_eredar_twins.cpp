...
struct MANGOS_DLL_DECL boss_alythessAI : public ScriptedAI
{
	...
	{	
        ...
        if(pInstance)
        {
            if(Creature* Sacrolash = ((Creature*)m_creature->GetMap()->GetUnit( pInstance->GetData64(DATA_SACROLASH))))
...
struct MANGOS_DLL_DECL boss_sacrolashAI : public ScriptedAI
{
	...
	{	
        ...
        if(pInstance)
        {
            if(Creature* Alythess = ((Creature*)m_creature->GetMap()->GetUnit( pInstance->GetData64(DATA_ALYTHESS))))
                if(!Alythess->isAlive())
                    Alythess->Respawn();
        }
        */
