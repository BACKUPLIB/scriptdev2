...
struct MANGOS_DLL_DECL mob_snobold_vassalAI : public BSWScriptedAI
{
    ...
    void Reset()
    {
        ...
        pBoss = (Creature*)m_creature->GetMap()->GetUnit(m_pInstance->GetData64(NPC_GORMOK));
...
struct MANGOS_DLL_DECL boss_acidmawAI : public BSWScriptedAI
{
    ...
    {
        if (!m_pInstance) return;
            if (Creature* pSister = (Creature*)m_creature->GetMap()->GetUnit(m_pInstance->GetData64(NPC_DREADSCALE)))
               if (!pSister->isAlive())
...
struct MANGOS_DLL_DECL boss_dreadscaleAI : public BSWScriptedAI
{
    ...
    {
        if (!m_pInstance) return;
            if (Creature* pSister = (Creature*)m_creature->GetMap()->GetUnit(m_pInstance->GetData64(NPC_ACIDMAW)))
               if (!pSister->isAlive())
                         m_pInstance->SetData(TYPE_NORTHREND_BEASTS, SNAKES_DONE);
                else m_pInstance->SetData(TYPE_NORTHREND_BEASTS, SNAKES_SPECIAL);
    }
