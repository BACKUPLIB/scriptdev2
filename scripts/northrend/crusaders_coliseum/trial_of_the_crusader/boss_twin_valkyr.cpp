...
struct MANGOS_DLL_DECL boss_fjolaAI : public BSWScriptedAI
{
    ...
    void JustDied(Unit* pKiller)
    {
        if (!m_pInstance) return;
            ...
            if (Creature* pSister = (Creature*)m_creature->GetMap()->GetUnit(m_pInstance->GetData64(NPC_DARKBANE)))
               if (!pSister->isAlive())
...
struct MANGOS_DLL_DECL boss_eydisAI : public BSWScriptedAI
{
    ...
    void JustDied(Unit* pKiller)
    {
        if (!m_pInstance) return;
            DoScriptText(-1713547,m_creature);
            if (Creature* pSister = (Creature*)m_creature->GetMap()->GetUnit(m_pInstance->GetData64(NPC_LIGHTBANE)))
               if (!pSister->isAlive())
                         m_pInstance->SetData(TYPE_VALKIRIES, DONE);
                else m_pInstance->SetData(TYPE_VALKIRIES, SPECIAL);
...
struct MANGOS_DLL_DECL mob_unleashed_darkAI : public ScriptedAI
{
    ...
    void Reset()
    {
        ...
        pboss1 = (Creature*)m_creature->GetMap()->GetUnit(m_pInstance->GetData64(NPC_DARKBANE));
        pboss2 = (Creature*)m_creature->GetMap()->GetUnit(m_pInstance->GetData64(NPC_LIGHTBANE));
    }
