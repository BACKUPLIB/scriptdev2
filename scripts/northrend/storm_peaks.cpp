...
enum
{
    ...
    NPC_SAY_EXIT            = -1039980,
    NPC_SAY_UNGRATEFUL_USER = -1039979,
    NPC_SAY_REMOVE_KILL     = -1039978,
...
bool GossipHello_npc_captive_mechagnome(Player* pPlayer, Creature* pCreature)
{
    if(pCreature && pPlayer && pPlayer->GetQuestStatus(QUEST_SLAVES_OF_STORMFORGED) == QUEST_STATUS_INCOMPLETE)
    {
        if(rand()%2)
        {
            pCreature->MonsterYell(GOSSIP_TEXT_MECHAGNOME1,LANG_UNIVERSAL,pPlayer);
            (( npc_captive_mechagnomeAI*)pCreature->AI())->StartFollow(pPlayer,pPlayer->getFaction());
        }
        else
        {
            pCreature->MonsterYell(GOSSIP_TEXT_MECHAGNOME2,LANG_UNIVERSAL,pPlayer);
...
void AddSC_storm_peaks()
{
    ...
    newscript->pGOUse = &GOHello_rusty_cage;
    newscript->RegisterSelf(false);
