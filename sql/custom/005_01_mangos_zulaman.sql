...
INSERT INTO `spell_proc_event` (`entry`,`procFlags`) VALUES ('43983','16384');
...
-- doors
DELETE FROM `gameobject` WHERE `id` IN (186303, 186304, 186859, 186858) AND `guid` >= 100000; -- only apply once
...
# Despawn 'Tanzar'
UPDATE `creature` SET `spawnMask` = 0 WHERE `id` = 23790;

# Zulaman Creature spawn on the way to akil'zon
DELETE FROM creature WHERE map=568 AND guid > 100000; -- only apply once
...
UPDATE `creature_template` SET `ScriptName` = 'boss_halazzi', `minhealth` = 600000, `maxhealth` = 600000 WHERE `entry` = '23577';
