-- event hallows end fixes

-- add new templates
DELETE FROM gameobject_template WHERE entry BETWEEN 301200 AND 301205;
INSERT INTO `gameobject_template` (`entry`,`type`,`displayId`,`name`,`size`,`data1`) VALUES 
(301200,2,6404,'Eimer mit Süßigkeiten',1,0), -- sanktum der sterne
(301201,2,6404,'Eimer mit Süßigkeiten',1,0), -- allerias feste
(301202,2,6404,'Eimer mit Süßigkeiten',1,0), -- ehrenfeste
(301203,2,6404,'Eimer mit Süßigkeiten',1,0), -- shattrat seher
(301204,2,6404,'Eimer mit Süßigkeiten',1,0), -- altar der shatar
(301205,2,6404,'Eimer mit Süßigkeiten',1,0); -- der ewige hain
...
DELETE FROM `gameobject` WHERE `id` IN (SELECT `entry` FROM `gameobject_template` WHERE `name` LIKE 'Eimer mit Süßigkeiten') AND `id` NOT IN (SELECT `id` FROM `gameobject_questrelation`);
DELETE FROM `game_event_gameobject` WHERE `event` ='12' AND `guid` NOT IN (SELECT `guid` FROM `gameobject`);

DELETE FROM `game_event_gameobject` WHERE `guid` BETWEEN 99755 AND 99757;
INSERT INTO `game_event_gameobject` (`guid`, `event`) VALUES
('99755', '12'),
('99756', '12'),
('99757', '12');
